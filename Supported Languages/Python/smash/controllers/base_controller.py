...
"""
    smashcontrollers.base_controller
...
class BaseController(object):
    ...
    def validate_response(self, context):
        ...
        if context.response.status_code == 400:
            ...
            raise APIException('Incorrect Credentials', context)
        elif context.response.status_code == 403:
            raise APIException('Forbidden', context)
        elif context.response.status_code == 406:
            raise APIException('Not Acceptable Input', context)
        elif context.response.status_code == 409:
            raise APIException('Data Conflict', context)
        elif context.response.status_code == 413:
            raise APIException('Payload Too Large', context)
        elif context.response.status_code == 415:
            raise APIException('Un-Supported Content Type', context)
        elif context.response.status_code == 451:
            raise APIException('Un-Available For Legal Reasons', context)
        elif context.response.status_code == 500:
            raise APIException('Internal API Error', context)
        elif context.response.status_code == 501:
            raise APIException('Not Implemented', context)
        elif context.response.status_code == 503:
            raise APIException('API Unavailable', context)
        elif context.response.status_code == 504:
            raise APIException('Request Timeout', context)
        elif context.response.status_code == 511:
            raise APIException('Authentication Required', context)
